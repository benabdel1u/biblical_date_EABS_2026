# sefaria_merge_v20.py
#
# v18 adds the adaptation layer required by the real Pentateuch spreadsheets.
# Everything v17 fixed is retained. See CHANGELOG_v18.txt.
#
# What changed, in one line each:
#   - Hebrew header sits on the second row of the sheet.
#   - Book names differ on each side (Genesis / Gen) -> canonical keys.
#   - Syriac morphology is parsed BY PREFIX, not by column position.
#   - Hebrew (BHSA) and Syriac (CALAP) tag vocabularies are now mapped.
#   - wayyiqtol is NEUTRAL by default: it earns no tense bonus, and loses none.
#   - Missing Syriac glosses are detected and reported; an external gloss file
#     can be plugged in without touching the code.
...
OUTPUT_FILE = 'merged_verbs_results_v20.csv'
...
# CALIBRATED, July 2026. These are no longer guesses.
#
# Method: 99 distinct (Hebrew lexeme x Syriac lemma) pair types were annotated by hand,
# stratified by cosine band and balanced across qal / derived stems. Every annotated pair
# lay in the 0.31-0.57 range, i.e. BELOW the old 0.60 threshold. Result:
#
#     cosine band   n    precision
#     0.30 - 0.35    9      56%     <- the break
#     0.35 - 0.40   27      89%
#     0.40 - 0.45   14     100%
#     0.45 - 0.55   44      95%
#     overall       99      91%
#
# The old threshold of 0.60 was therefore discarding pairs that were correct 91% of the
# time. It cost 2,618 matches on the Pentateuch. Precision collapses below 0.35, and that
# is where the threshold now sits.
#
# TRAP: the acceptance rule requires BOTH conditions. Lowering MIN_BERT_SEMANTIC_ONLY while
# leaving MIN_TOTAL_SCORE at 0.60 changes nothing, because a pair scoring 0.39 with no
# morphological bonus totals 0.39. The two must move together.
MIN_BERT_SEMANTIC_ONLY = 0.35
MIN_TOTAL_SCORE = 0.35

# Estimated effect on the full Pentateuch (occurrence-weighted precision on the annotated
# bands; above 0.57 precision is presumed higher, so this is conservative):
#     threshold   matched   rate     precision   expected false positives
#     0.60         14,544   80.4%       -              -
#     0.35         17,162   94.9%      98%          ~157  (1% of matches)
...
def accept_assignment(total, comps):
    ...
    THE PREMISE WAS WRONG. Hand annotation (July 2026) marks halak x CPR as a CORRECT
